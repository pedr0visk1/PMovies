...
async function getMovie() {
    ...
    detalhes.innerHTML = `<h1 class= 'text-green'>${movie.title ?? movie.name}</h1>
    <h5 class='mb-4'> Título Original: 
    ${movie.original_title ?? movie.original_name} </h5>
    <p class='mb-1' ><b>Data de Estreia: </b> ${movie.relase_date}</p>
    ...
    movie.genres.forEach(genre => {
        detalhes.innerHTML += `<button class='btn btn-lg btn-outline-success me-1'>${genre.name}</button>`
    });
    ...
    await fetch(`${baseUrl}${media}/${id}/credits?language=pt-br`, options)
        ...
        .catch(err => console.log('Erro ao carregar créditos', err));
    // console.log(elenco);
